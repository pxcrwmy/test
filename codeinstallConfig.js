...
function importHeaderFile(path){
	...
	// 插入 头文件
	var hasImport = rf.match(/RCTCodeInstall.h/);
	if (hasImport != null) {
		return
	}
...
function codeAppDelegate(path){
	...
	if(matchFinishLaunching != null){
		var hasCoded = rf.match(/CodeInstallSDK initWithDelegate/);
		if(hasCoded == null){
	...
	if(matchContinueUserActivity == null){
		...
		var hasCoded = rf.match(/CodeInstallSDK continueUserActivity/);
		if(hasCoded == null){
...
function writePlist(path){
	...
	var codeAppkey = rf.match(/<key>com.CodeInstall.APP_KEY<\/key>/);
	if(codeAppkey == null){
	...
	var codeScheme = rf.match(/<key>CFBundleURLName<\/key>\s*<string>codeinstall<\/string>/);
	if(codeScheme == null){
...
function configManifestXml(path){
	var err = false;
	var rf = fs.readFileSync(path, "utf-8");
	var alreadyConfigAppkey = rf.match(/com.CodeInstall.APP_KEY/);
	...
	var alreadyConfigScheme = rf.match(/<!-- codeinstall scheme -->/);
